  ...
  return defineConfig({
    ...
    resolve: {
      ...
      alias: [
        ...
        // Self-host: swap the Stripe SDK for an empty stub. The real package
        // has a module-init side effect that injects js.stripe.com/v3 as soon
        // as it's imported — runtime gating is too late. Aliasing at build
        // time keeps the script from ever being loaded.
        ...(process.env.VITE_SELF_HOSTED === "true"
          ? [
              { find: "@stripe/stripe-js", replacement: "/src/stubs/stripe-js-stub.ts" },
              { find: "@stripe/react-stripe-js", replacement: "/src/stubs/react-stripe-js-stub.tsx" },
            ]
          : []),
      ],
    },
