...
import { isBackendEnvBeta, isBackendEnvEmulator, isSelfHosted } from "utils/EnvUtils";
...
const generateEndpointPrefix = (username: string = null, teamId?: string, collectionPath?: string) => {
  ...
  if (isSelfHosted()) {
    // The constant uses `__SELF_HOST_LOCALHOST__` as a runtime placeholder when
    // no HTTPS functions URL is configured at build time — swap it for the
    // current page's hostname so the docker default still works.
    const base = APP_CONSTANTS.mock_base_url.selfHosted.replace(
      "__SELF_HOST_LOCALHOST__",
      `http://${window.location.hostname || "localhost"}`,
    );
    prefix = `${base}/`;
  } else if (isBackendEnvBeta()) {
...
const generateEndpointSuffix = (username: string, uid?: string, teamId?: string, password?: string) => {
  ...
  } else if (!username || isBackendEnvBeta() || isBackendEnvEmulator() || isSelfHosted()) {
