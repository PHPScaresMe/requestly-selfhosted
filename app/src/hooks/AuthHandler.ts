...
const AuthHandler: React.FC<{}> = () => {
  ...
  const nonBlockingOperations = useCallback(
    async (user: User) => {
      ...
      if (!isSelfHosted() && user?.metadata?.creationTime === user?.metadata?.lastSignInTime) {
        if (
      ...
      const enterpriseDetails: any = isSelfHosted()
        ? { data: { enterpriseData: null } }
        : await getEnterpriseAdminDetails();
