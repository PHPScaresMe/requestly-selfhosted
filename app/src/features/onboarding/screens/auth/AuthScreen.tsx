...
import { SelfHostedAuthForm } from "./components/SelfHostedAuthForm/SelfHostedAuthForm";
  ...
  const handlePostAuthSyncVerification = useCallback(
    (metadata: AuthSyncMetadata["syncData"]) => {
      ...
      if (metadata.forceBstackAuth || metadata.isSyncedUser) {
        ...
        // Self-host has no SMTP wired by default, so the magic-link sign-in flow
        // can't deliver. Force the password form for every existing-user path.
        if (isSelfHosted()) {
          if (metadata.providers.length === 0) setAuthProviders([AuthProvider.PASSWORD]);
          setShowRQAuthForm(true);
        } else if (metadata.providers.length === 1 && metadata.providers[0] === AuthProvider.PASSWORD && !isDesktopSignIn) {
          handleSendEmailLink();
        } else {
