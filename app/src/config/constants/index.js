...
// Self-hosted instances route mock requests through the same functions emulator
// we use for everything else. If `VITE_FIREBASE_FUNCTIONS_URL` is set (HTTPS
// subdomain topology) we derive the mock base from it; otherwise we fall back
// to the localhost-on-port-5001 docker default. The bare hostname for the
// localhost fallback is filled in at runtime in mocksV2/utils — leaving the
// placeholder here keeps this file environment-agnostic.
const selfHostedFunctionsBase =
  process.env.VITE_FIREBASE_FUNCTIONS_URL?.replace(/\/$/, "") ?? "__SELF_HOST_LOCALHOST__:5001";
...
APP_CONSTANTS.mock_base_url = {
  ...
  selfHosted: `${selfHostedFunctionsBase}/requestly-self-hosted/us-central1/handleMockRequest/api/mockv2`,
};
