...
export async function signUp(email, password, refCode, source) {
  ...
  return createUserWithEmailAndPassword(auth, email, password)
    .then((result) => {
      // In self-host the seedUserDocs trigger already flipped emailVerified=true on
      // the auth account, and SMTP is reserved for password resets / sharing rather
      // than nag emails. Skip the verification mail entirely.
      if (!isSelfHosted()) {
...
export function forgotPassword(email) {
  ...
  // In self-host the SDK's reset email is intercepted by the Auth Emulator and never
  // delivered. Use our custom cloud function that generates the same oobCode via the
  // admin SDK and mails it over the configured SMTP relay.
  if (isSelfHosted()) {
    const sendReset = httpsCallable(getFunctions(), "auth-sendPasswordReset");
    return sendReset({ email })
      .then(() => {
        trackForgotPasswordSuccessEvent({ email });
        return { status: true, msg: "Please check your email for instructions to reset your password." };
      })
      .catch((error) => {
