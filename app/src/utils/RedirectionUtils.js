...
import { isSelfHosted } from "./EnvUtils";
...
export const redirectToOAuthUrl = (navigate) => {
  if (isSelfHosted()) {
    // No external OAuth in self-host — just open the auth modal in sign-up mode and
    // let SelfHostedAuthForm handle email/password registration inline.
    reduxStore.dispatch(
      globalActions.toggleActiveModal({
        modalName: "authModal",
        newValue: true,
        newProps: { authMode: APP_CONSTANTS.AUTH.ACTION_LABELS.SIGN_UP },
      })
    );
    return;
  }
